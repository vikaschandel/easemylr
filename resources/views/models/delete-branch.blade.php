<div class="modal fade" id="deletebranch" tabindex="-1" role="dialog" aria-hidden="true">
   <div class="modal-dialog">
     <div class="modal-content">
       <!-- <button type="button" class="close" data-dismiss="modal"><img src="{{asset('assets/img/close-bottle.png')}}" class="img-fluid"></button> -->
       <!-- Modal Header -->
       <div class="modal-header text-center">
        <h4 class="modal-title">Delete</h4>
       </div>
       <!-- Modal body -->
       <div class="modal-body">
          <div class="Delt-content text-center">
             <img src="{{asset('assets/img/delte.png')}}" class="img-fluid mb-2">
             <h5 class="my-2">Are you sure you want to delete this branch?</h5>
          </div>
       </div>
       <!-- Modal footer -->
       <div class="modal-footer">
           <div class="btn-section w-100 P-0">
               <a class="btn-cstm btn-white btn btn-modal delete-btn-modal deletebranchconfirm">Yes</a> 
               <a type="" class="btn btn-modal" data-dismiss="modal">No</a>
           </div>
       </div>
     </div>
   </div>
</div>